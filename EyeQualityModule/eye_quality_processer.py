...
from .models import DenseNet121_v0
import torch
...
class EyeQualityProcesser():
    ...
    def __call__(self, img,save_path=None):
        ...
        pre = self.model(img_rgb,img_hsv,img_lab)

        _, _, _, _,predict = (pre.data).cpu().detach()
        pred=torch.max(pred[0],dim=0)[1]
        print(pred)
        if save_path:
